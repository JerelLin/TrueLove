...
import "../../../stylesheets/marriage_component/auth/login.css";
	...
	handleLogin(event){
		...
		if(phoneNumber == null || phoneNumber.length == 0){
			if(password == null || password.length == 0){
				...
				this.setState({ errMess : "亲~手机号码不能为空哦~" });
				this.refs.errMess.style.display="block";
				return false;
			};
		}else if(password == null || password.length == 0){
			this.setState({ errMess : "亲~密码不能为空哦~" });
			this.refs.errMess.style.display = "block";
			return false;
			...
			.end(function(err, res){
				...
				setTimeout(function(){
					switch(parseInt(res.body.auth)){
						...
						case 1:
							//跳转到 审核中 界面
							_this.props.history.replaceState(null, '/marriage_app/checking');
							break;
						case 2:
							//跳转到 应用主页 界面
							...
							break;
						default :
							_this.setState({ errMess : "呀~ 好像出了点问题, 亲重新再登录试试? " });
							break;
					};
				}, 1000);
			});
