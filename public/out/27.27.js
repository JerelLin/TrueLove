	...
	var News = function (_React$Component) {
		...
		_createClass(News, [{
			...
			value: function componentDidMount() {
				var _this = this;
				_superagent2.default.get("/initNews").accept('application/json').end(function (err, res) {
					if (err || !res.ok) {
						console.log(err);
						return false;
					};
			...
			value: function news_delete(timestamp) {
				_superagent2.default.post("/deleteNews").type('application/json') /*发送什么类型的数据*/
				.send({
					timestamp: timestamp
					...
					console.log("已经删除时间戳为 " + timestamp + " 的消息");
				});
			}
		}, {
			key: "queryByPage",
			value: function queryByPage(page) {
				_superagent2.default.get("/getPageNews").accept('application/json').query({ page: page }).end(function (err, res) {
					if (err || !res.ok) {
						console.log(err);
						return false;
					};
					console.log("获取第" + page + "页消息");
				});
			}
		}, {
			key: "render",
					...
					_react2.default.createElement(
						...
						this.state.news ? this.state.news.map(function (element, index) {
							return _react2.default.createElement(
								...
								_react2.default.createElement(
									_reactRouter.Link,
									{ to: "/marriage_app/News_detail/" + element.timestamp },
									_react2.default.createElement(
										"span",
